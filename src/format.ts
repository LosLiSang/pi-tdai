...
export function formatRecallContext(input: RecallFormatInput): RecallFormatResult {
  const maxChars = Math.max(1000, input.maxContextChars);
  const sections: string[] = [];
  let remaining = maxChars - 700;

  const addSection = (title: string, content: string, preferredLimit: number): boolean => {
    if (remaining <= 100 || !content.trim()) return false;
    const body = truncateText(content.trim(), Math.min(preferredLimit, remaining));
    sections.push(`## ${title}\n${body}`);
    remaining -= body.length + title.length + 8;
    return true;
  };

  const hasPersona = Boolean(input.persona?.trim()) && addSection(
  ...
  for (const [index, memory] of input.memories.entries()) {
    ...
    const meta = [memory.type, memory.score == null ? undefined : `score=${memory.score.toFixed(3)}`]
      .filter(Boolean)
      .join(", ");
  ...
  const memoryAdded = memoryLines.length > 0 && addSection("L1 Relevant Memories", memoryLines.join("\n"), remaining);

  const scenarioLines: string[] = [];
  let scenarioChars = 0;
  for (const scenario of input.scenarios) {
    if (remaining <= 200) break;
    const summary = scenario.summary ? ` — ${truncateText(escapeMemoryText(scenario.summary), 300)}` : "";
  ...
  const scenariosAdded = scenarioLines.length > 0 && addSection(
